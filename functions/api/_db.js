...
export async function getRegulacaoSchemaStatus(env){
  if(!env.DB_REGULACAO)return{bindingOk:false,schemaOk:false,tabelasExistentes:[],tabelasFaltantes:[...REGULACAO_TABLES],colunasPacienteEnderecoFaltantes:[...PACIENTE_ENDERECO_COLUMNS],colunasPacienteIntegracaoFaltantes:['cns','nome_social','identidade_genero'],colunasFluxoV210Faltantes:['guias.codigo_guia'],colunasAgendaFaltantes:['especialidades.duracao_padrao_min'],erro:'Binding DB_REGULACAO não configurado.'};
  try{
    ...
    if(existentes.has('pacientes')){const enderecoStatus=await getPacienteEnderecoColumnStatus(env);colunasPacienteEnderecoFaltantes=enderecoStatus.faltantes;const info=await env.DB_REGULACAO.prepare("PRAGMA table_info('pacientes')").all();const cols=new Set((info.results||[]).map(c=>c.name));if(!cols.has('cns'))colunasPacienteIntegracaoFaltantes.push('cns');if(!cols.has('nome_social'))colunasPacienteIntegracaoFaltantes.push('nome_social');if(!cols.has('identidade_genero'))colunasPacienteIntegracaoFaltantes.push('identidade_genero')}
  ...
  }catch(err){return{bindingOk:true,schemaOk:false,tabelasExistentes:[],tabelasFaltantes:[...REGULACAO_TABLES],colunasPacienteEnderecoFaltantes:[...PACIENTE_ENDERECO_COLUMNS],colunasPacienteIntegracaoFaltantes:['cns','nome_social','identidade_genero'],colunasFluxoV210Faltantes:['guias.codigo_guia'],colunasAgendaFaltantes:['especialidades.duracao_padrao_min'],erro:messageOf(err)}}
}
