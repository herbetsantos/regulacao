...
export async function onRequestGet({request,env}){
  ...
  const required=[
    'regulacao_principals','regulacao_superusers','regulacao_auth_sessions',
    'regulacao_local_users','regulacao_login_attempts','regulacao_unidades',
    'regulacao_equipes','regulacao_equipe_unidades','regulacao_principal_acessos',
    'regulacao_principal_unidades','regulacao_principal_equipes','regulacao_profissionais',
    'regulacao_profissional_vinculos','regulacao_profissional_equipes',
    'agenda_grupo_profissionais','regulacao_etiquetas','guia_etiquetas',
    'regulacao_execucoes_administrativas'
  ];
  const missingTables=[];
  for(const t of required)if(!await tableExists(env.DB_REGULACAO,t))missingTables.push(t);
  ...
  for(const[t,c]of requiredColumns){
    if(await tableExists(env.DB_REGULACAO,t)&&!await columnExists(env.DB_REGULACAO,t,c)){
  ...
  const ready=
    ...
    versionAtLeast(version,MIN_ADMIN_SCHEMA);

  return json({
    version,
    minimum_version:MIN_ADMIN_SCHEMA,
    ready_current:ready,
    ready_2_26_3:ready,
    ready_2_26:ready,
    ready_2_25:ready,
