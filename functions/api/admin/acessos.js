...
async function fullRow(env,pid,base){
  const a=await env.DB_REGULACAO.prepare('SELECT cadastrante,regulador,organizador,executor,gestor,administrador,active FROM regulacao_principal_acessos WHERE principal_id=?').bind(pid).first();
  ...
  return{principal_id:pid,...base,cadastrante:!!a?.cadastrante,regulador:!!a?.regulador,organizador:!!a?.organizador,executor:!!a?.executor,gestor:!!a?.gestor,administrador:base.source==='portal'&&base.portal_role==='super_admin'?true:!!a?.administrador,access_active:a?!!a.active:true,unidades:units||[],equipe_id:team?.equipe_id??null};
}
...
export async function onRequestGet({request,env}){
  ...
  if(funcao){filtered=filtered.filter(x=>funcao==='sem_funcao'?!(x.cadastrante||x.regulador||x.organizador||x.executor||x.gestor||x.administrador):!!x[funcao])}
