...
export async function onRequestPut({ request, env, params }) {
  ...
  const atual = await env.DB_REGULACAO.prepare('SELECT id, nome, ativo, sort_order FROM especialidades WHERE id = ?').bind(id).first();
  ...
  await env.DB_REGULACAO.prepare('UPDATE especialidades SET nome = ?, ativo = ?, sort_order = ? WHERE id = ?')
    .bind(nome, ativo, Number.isFinite(sortOrder) ? sortOrder : atual.sort_order, id).run();
